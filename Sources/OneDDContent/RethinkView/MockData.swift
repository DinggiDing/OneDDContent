...
// PageIntroProvider 클래스 정의
class PageIntroProvider: ObservableObject {
    @Published var pageIntros: [PageIntro] = []
    ...
    init(locale: Locale) {
        self.pageIntros = getPageIntros(for: locale)
    }
    
    private func getPageIntros(for locale: Locale) -> [PageIntro] {
        switch locale.identifier {
        case "ko":
            return pageIntros_ko
        case "ja":
            return pageIntros_ja
        case "en":
            return pageIntros_en
        default:
            return pageIntros_en // default to English if locale is not recognized
        }
    }
}
